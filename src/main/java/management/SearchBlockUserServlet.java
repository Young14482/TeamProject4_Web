...
public class SearchBlockUserServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try (SqlSession sqlSession = app.getSqlSession()) {
			...
			} else if ("BlockUserGrade".equals(searchField)) {
				// 회원 등급 별 검색된 차단 회원 리스트
				searchBlackList = manageMapper.getSelectBlockByGrade(searchText);
				// 회원 등급 별 검색된 차단 회원 수
				searchBlockUserCount = manageMapper.getCountBlockByGrade(searchText);
				
			} else if ("BlockUserGender".equals(searchField)) {
				// 성별 별 검색된 차단 회원 리스트
				searchBlackList = manageMapper.getSelectBlockByGender(searchText);
				// 성별 별 검색된 차단 회원 수
				searchBlockUserCount = manageMapper.getCountBlockByGender(searchText);
			}
		}
