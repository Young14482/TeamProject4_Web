...
<body>
	...
	<div class="usage-container">
		<div id="usage1" class="usage" onclick="selectUsage('1')">
			<img src="data:image/png;base64,<%=session.getAttribute("운동")%>" alt="운동">
			<p>운동</p>
		</div>
		<div id="usage2" class="usage" onclick="selectUsage('2')">
			<img src="data:image/png;base64,<%=session.getAttribute("경사")%>" alt="경사">
			<p>경사</p>
		</div>
		<div id="usage3" class="usage" onclick="selectUsage('3')">
			<img src="data:image/png;base64,<%=session.getAttribute("캠핑")%>" alt="캠핑">
			<p>캠핑</p>
		</div>
		<div id="usage4" class="usage" onclick="selectUsage('4')">
			<img src="data:image/png;base64,<%=session.getAttribute("등산")%>" alt="등산">
			<p>등산</p>
		</div>
		<div id="usage5" class="usage" onclick="selectUsage('5')">
			<img src="data:image/png;base64,<%=session.getAttribute("비즈니스")%>" alt="비즈니스">
			<p>비즈니스</p>
		</div>
		<div id="usage6" class="usage" onclick="selectUsage('6')">
			<img src="data:image/png;base64,<%=session.getAttribute("일상")%>" alt="일상">
			<p>일상</p>
		</div>
		<div id="usage7" class="usage" onclick="selectUsage('7')">
			<img src="data:image/png;base64,<%=session.getAttribute("실내")%>" alt="실내">
			<p>실내</p>
		</div>
	...
	<div class="price-container">
	    <div id="priceRange" class="price-range" onclick="activatePriceRange()">
	        <label><input type="radio" name="priceOption" value="range"> 가격대 선택</label>
	        <div class="price-options">
	            <label><input type="radio" name="price" value="7000 이하" data-min="0" data-max="7000" disabled> 7천원 이하</label>
	            <label><input type="radio" name="price" value="7000-14000" data-min="7000" data-max="14000" disabled> 7천원~1만 4천원</label>
	            <label><input type="radio" name="price" value="14000-21000" data-min="14000" data-max="21000" disabled> 1만 4천원~2만 1천원</label>
	            <label><input type="radio" name="price" value="21000-28000" data-min="21000" data-max="28000" disabled> 2만 1천원~2만 8천원</label>
	            <label><input type="radio" name="price" value="28000 이상" data-min="28000" data-max="999999" disabled> 2만 8천원 이상</label>
	        </div>
	    </div>
	    <div id="customPrice" class="custom-price" onclick="activateCustomPrice()">
		    <label class="custom-price-label"><input type="radio" name="priceOption" value="custom"> 직접 입력</label>
	        <div class="custom-price-inputs">
	            <input type="number" id="minPrice" name="minPrice" placeholder="최소 금액" disabled required>
	            ~
	            <input type="number" id="maxPrice" name="maxPrice" placeholder="최대 금액" disabled required>
	        </div>
	    </div>
	    <div id="allPrice" class="all-price" onclick="activateAllPrice()">
	        <label><input type="radio" name="priceOption" value="all"> 모든 가격</label>
	    </div>
	</div>
